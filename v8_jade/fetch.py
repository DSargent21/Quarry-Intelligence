...
def load_data(use_cache=True, cache_path=CACHE, verbose=True):
    ...
    leagues = fetch_all("leagues", "id, name, sport")
    bet_types = fetch_all("bet_types", "id, name")
    cappers = fetch_all("capper_directory", "id, canonical_name")
    sources = fetch_all("pick_sources", "id, source_platform, original_message, ocr_text")
